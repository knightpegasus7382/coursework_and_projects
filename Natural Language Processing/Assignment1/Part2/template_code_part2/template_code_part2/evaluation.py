...
class Evaluation():
	...
	def queryPrecision(self, query_doc_IDs_ordered, query_id, true_doc_IDs, k):
		...
		#Fill in code here
		#print(query_doc_IDs_ordered, true_doc_IDs)
	...
	def queryNDCG(self, query_doc_IDs_ordered, query_id, true_doc_IDs, k):
		...
		req_relevance_scores = np.zeros(len(returned_doc_IDs))

		for true_doc_ID in true_doc_IDs:
			if int(true_doc_ID['id']) in returned_doc_IDs:
				req_relevance_scores[returned_doc_IDs.index(int(true_doc_ID['id']))] = 5-true_doc_ID['position']

		#print(returned_doc_IDs, req_relevance_scores)

		discounts = np.log(np.arange(1, len(req_relevance_scores)+1) + 1)/np.log(2)
		DCG = np.sum(req_relevance_scores/discounts)

		sorted_req_relevance_scores = np.sort(req_relevance_scores)[::-1]
		IDCG = np.sum(sorted_req_relevance_scores/discounts)
