...
def q_update(s, a, r, s_, Q, alpha, gamma):
	delta = r + gamma*np.max(Q[:,s_[0],s_[1]]) - Q[a,s[0],s[1]]
	Q[a,s[0],s[1]] = Q[a,s[0],s[1]] + alpha*delta
	return Q

# Function to do a single run of Q-Learning
def q_learn(gamma, alpha, eps, n_episodes, env, goal):
	...
	for n in range(episodes):
		...
		for steps in range(1000):
			...
			Q = q_update(s, a, r, s_, Q, alpha, gamma)
			steps_list[n] += 1
			r_list[n] += r
			s = s_
			a = a_
			if (s == goal_pos).all(): # If the goal state is reached:
				break
...
def plots(avg_r, avg_steps, episodes, goal, goalpos, pi):
	...
	plt.title('Q-LEARNING: Average reward vs Episodes: Goal '+ goal)
	plt.ylabel('Average Reward \u279d')
	plt.xlabel('Episodes \u279d')
	plt.legend(loc = 'lower right')
	fig2=plt.figure(figsize=(8,5))
	plt.plot(range(episodes), avg_steps, 'r', label = "Steps")
	plt.title('Q-LEARNING: Average steps vs Episodes: Goal '+ goal)
	plt.ylabel('Steps \u279d')
	plt.xlabel('Episodes \u279d')
	...
	fig, ax = plt.subplots()
	ax.set_title('Q-LEARNING: Optimal Policy for Goal '+goal)
	ax.set_xticks([])
	ax.set_yticks([])
	for i in range(12):
		for j in range(12):
	...
	ax.matshow(pi, cmap = plt.cm.Pastel2)
	plt.show()
...
def main():
	...
	for ind, goalpos in enumerate(goal_positions):
		goal = goal_list[ind]
		print("Q-LEARNING: Learning for goal " + goal+"...")
		runs = np.array([(q_learn(gamma, alpha, eps, episodes, gym.make('gym_puddle:puddle-v0'),goal)) for i in range(50)])
