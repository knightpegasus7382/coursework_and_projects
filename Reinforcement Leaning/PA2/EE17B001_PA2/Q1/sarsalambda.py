...
# Function to make a SARSA-lambda Update
def sarsa_lambda_update(s, a, r, s_, a_, Q, el, alpha, gamma):
	delta = r + gamma*Q[a_,s_[0],s_[1]] - Q[a,s[0],s[1]]
	Q = Q + alpha*delta*el # The Q function is updated for ALL s,a using eligibility traces
	return Q

# Function to do a single run of SARSA-lambda
def sarsa_lambda_learn(gamma, lmda, alpha, eps, n_episodes, env, goal):
	Q = rndm.rand(env.action_space.n, env.observation_space.shape[0], env.observation_space.shape[1])
	el = np.zeros((env.action_space.n, env.observation_space.shape[0], env.observation_space.shape[1]))
	...
	for n in range(episodes):
		...
		for steps in range(1000):
			...
			for k in range(env.action_space.n):
				if k!=a:
					el[k, s[0], s[1]] = 0
			el = el * (el > 0.05) # Truncating traces
			Q = sarsa_lambda_update(s, a, r, s_, a_, Q, el, alpha, gamma)
			steps_list[n] += 1
			r_list[n] += r
			s = s_
			a = a_
...
# Function to plot the average reward and average steps plots (even for the first 25 learning trials), and policy
def plots(avg_r, avg_steps, episodes, goal, goalpos, pi, lmda):
	fig1=plt.figure(figsize=(8,5))
	plt.plot(range(episodes), avg_r, 'b' , label = "Average Reward" )
	plt.title('SARSA('+r'$\lambda = $'+str(lmda)+'): Average Reward vs Episodes: Goal '+ goal)
	plt.ylabel('Average Reward \u279d')
	plt.xlabel('Episodes \u279d')
	plt.legend(loc = 'lower right')
	fig2=plt.figure(figsize=(8,5))
	plt.plot(range(episodes), avg_steps, 'r', label = "Steps")
	plt.title('SARSA('+r'$\lambda = $'+str(lmda)+'): Average Steps vs Episodes: Goal '+ goal)
	plt.ylabel('Steps \u279d')
	plt.xlabel('Episodes \u279d')
	...
	fig, ax = plt.subplots()
	ax.set_title('SARSA('+r'$\lambda = $'+str(lmda)+'): Optimal Policy for Goal '+goal)
	for i in range(12):
		for j in range(12):
			if [j,i] == goalpos:
	...
	plt.show()
	fig1=plt.figure(figsize=(8,5))
	plt.plot(range(25), avg_r[0:25], 'b' , label = "Average Reward" )
	plt.title('SARSA('+r'$\lambda = $'+str(lmda)+'): Average Reward vs Episodes (First 25 Learning Trials): Goal '+ goal)
	plt.ylabel('Average Reward \u279d')
	plt.xlabel('Episodes \u279d')
	plt.legend(loc = 'lower right')
	fig2=plt.figure(figsize=(8,5))
	plt.plot(range(25), avg_steps[0:25], 'r', label = "Steps")
	plt.title('SARSA('+r'$\lambda = $'+str(lmda)+'): Average Steps vs Episodes (First 25 Learning Trials): Goal '+ goal)
	plt.ylabel('Steps \u279d')
	plt.xlabel('Episodes \u279d')
	plt.legend(loc = 'upper right')
	plt.show()
...
# Function to run the 50 parallel runs for all goals: A, B, and C
def all_goals(lmda):
	print("SARSA (lambda = "+str(lmda)+")...")
	goal_list = ['A','B','C']
	...
	for ind, goalpos in enumerate(goal_positions):
		goal = goal_list[ind]
		print("Learning for goal " + goal+"...")
		runs = np.array([(sarsa_lambda_learn(gamma, alpha, eps, episodes, gym.make('gym_puddle:puddle-v0'),goal)) for i in range(50)])
		...
		plots(average_rewards, average_steps, episodes, goal, goalpos, pi, lmda)


# Main Function with the option to choose from the SARSA-lambda options (comment out unneeded function calls)
def main():
	#all_goals(lmda = 0)
	all_goals(lmda = 0.3)
	#all_goals(lmda = 0.5)
